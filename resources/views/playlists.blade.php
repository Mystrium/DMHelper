...
<div class="container my-5">
    ...
    <div class="row" id="lists_container">
                        ...
                        @if($my)
                            <div class="flex position-absolute bottom-0 start-0 ms-2 mb-2">
                                <form action="/playlist/visible/{{ $playlist->id }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <button class="btn btn-outline-dark" submit>
                            ...
                            <button onclick='changeList(event, @json($playlist))' data-bs-toggle="modal" data-bs-target="#changePlaylistModal" type="submit" class="btn btn-warning btn-sm position-absolute bottom-0 end-0 mb-2 me-5">
...
@section('scripts')
    <script>
        ...
        async function loadMoreLists(isNewSearch = false) {
            ...
            if (isNewSearch) {
                page = 1;
                document.getElementById('lists_container').innerHTML = '';
            } else {
                page++;
            }
            ...
            try {
                @if($my)
                    const response = await fetch(`/playlists/fetch?page=${page}&query=${searchQ}&my=true`);
                @else
                    const response = await fetch(`/playlists/fetch?page=${page}&query=${searchQ}`);
                @endif
                ...
                if (data.data.length > 0) {
                    data.data.forEach(game => {
                        ...
                        gameCard.innerHTML = createListCard(game);
                        document.querySelector('#lists_container').appendChild(gameCard);
                    });
        ...
        function createListCard(game) {
            return `
            <a href="/music/${game.id}" style="text-decoration: none">
                <div class="game-card" style="background-image: url('https://img.youtube.com/vi/${game.music && game.music.length > 0 ? game.music[Math.floor(Math.random() * game.music.length)].youtube_url : ''}/0.jpg');">
                    <div class="game-card-overlay">
                        <div><h3 class="text-white">${game.title}</h3></div>
                        <div><h7 class="text-white">${game.description}</h7></div>
                    </div>
                    ...
                    @if($my)
                        <div class="flex position-absolute bottom-0 start-0 ms-2 mb-2">
                            ...
                            ` : `
                                <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="white" class="bi bi-eye-slash" viewBox="0 0 16 16">
                                    <path d="M13.359 11.238C15.06 9.72 16 8 16 8s-3-5.5-8-5.5a7 7 0 0 0-2.79.588l.77.771A6 6 0 0 1 8 3.5c2.12 0 3.879 1.168 5.168 2.457A13 13 0 0 1 14.828 8q-.086.13-.195.288c-.335.48-.83 1.12-1.465 1.755q-.247.248-.517.486z"/>
                                    <path d="M11.297 9.176a3.5 3.5 0 0 0-4.474-4.474l.823.823a2.5 2.5 0 0 1 2.829 2.829zm-2.943 1.299.822.822a3.5 3.5 0 0 1-4.474-4.474l.823.823a2.5 2.5 0 0 0 2.829 2.829"/>
                                    <path d="M3.35 5.47q-.27.24-.518.487A13 13 0 0 0 1.172 8l.195.288c.335.48.83 1.12 1.465 1.755C4.121 11.332 5.881 12.5 8 12.5c.716 0 1.39-.133 2.02-.36l.77.772A7 7 0 0 1 8 13.5C3 13.5 0 8 0 8s.939-1.721 2.641-3.238l.708.709zm10.296 8.884-12-12 .708-.708 12 12z"/>
                                </svg>
                        ...
                        <div class="flex position-absolute bottom-0 end-0 mb-2">
                            <button onclick='changeList(event, ${JSON.stringify(game)})' data-bs-toggle="modal" data-bs-target="#changePlaylistModal" type="submit" class="btn btn-warning btn-sm me-5">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pen" viewBox="0 0 16 16">
                                    <path d="m13.498.795.149-.149a1.207 1.207 0 1 1 1.707 1.708l-.149.148a1.5 1.5 0 0 1-.059 2.059L4.854 14.854a.5.5 0 0 1-.233.131l-4 1a.5.5 0 0 1-.606-.606l1-4a.5.5 0 0 1 .131-.232l9.642-9.642a.5.5 0 0 0-.642.056L6.854 4.854a.5.5 0 1 1-.708-.708L9.44.854A1.5 1.5 0 0 1 11.5.796a1.5 1.5 0 0 1 1.998-.001m-.644.766a.5.5 0 0 0-.707 0L1.95 11.756l-.764 3.057 3.057-.764L14.44 3.854a.5.5 0 0 0 0-.708z"/>
                                </svg>
                            </button>

                            <form method="POST" action="/playlist/${game.id}" class="position-absolute bottom-0 end-0" onsubmit="return confirmDeletion(event, '${game.title}');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger btn-sm me-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16">
