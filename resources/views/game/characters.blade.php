...
@php $can_edit = !request()->has('play'); @endphp
    <div class="container my-5">
        @if($can_edit)
            <a class="btn btn-success position-fixed bottom-0 end-0 m-2" href="/story/{{$gameId}}">{{__('links.story')}} -></a>
        ...
        <h3 class="my-3">{{__('labels.charlists')}}</h3>
        <div class="row">
            @foreach($characters as $character)
                <div class="col-md-3">
                    <div class="card ps-2 pt-1" onclick="showCharacterList(event, {{ $character['id'] }})" id="character_{{ $character['id'] }}">
                        ...
                        <p><span>{{ $character['race'] }}</span> {{ $character['class'] }} (lvl: <span id="level_span_{{$character['id']}}">{{ $character['level'] }}</span>)</p>
                        <p><span>
                        ...
                        </span> <span id="at_span_{{$character['id']}}">{{ $character['at'] }}</span></p>
                        <p><span>
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-heart" viewBox="0 0 16 16">
                                <path d="m8 2.748-.717-.737C5.6.281 2.514.878 1.4 3.053c-.523 1.023-.641 2.5.314 4.385.92 1.815 2.834 3.989 6.286 6.357 3.452-2.368 5.365-4.542 6.286-6.357.955-1.886.838-3.362.314-4.385C13.486.878 10.4.28 8.717 2.01zM8 15C-7.333 4.868 3.279-3.04 7.824 1.143q.09.083.176.171a3 3 0 0 1 .176-.17C12.72-3.042 23.333 4.867 8 15"/>
                            </svg>
                        </span> <span id="hp_span_{{$character['id']}}">{{ $character['hp'] }}</span>/{{ $character['max_hp'] }}</p>

                        <div class="bottom-0 end-0 mb-2">
                            @if($is_owner)
                                <button class="btn btn-warning" onclick='editCharacter(event, {{$character["id"]}})' data-bs-toggle="modal" data-bs-target="#editCharacterModal" type="submit" class="btn btn-warning btn-sm me-5">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="pdfModalLabel">{{__('labels.charlist')}}</h5>
...
@if($is_owner)
    <div class="modal fade" id="editCharacterModal" tabindex="-1" aria-labelledby="editCharacterModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="editCharacterModalLabel"></h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="editCharacterForm" method="POST">
                        ...
                        <div class="mb-3">
                            <label for="characterHp" class="form-label">{{ __('fields.hp') }}</label>
                            <input type="number" class="form-control" id="characterHp" name="hp" required>
                        </div>
                        <div class="mb-3">
                            <label for="characterAt" class="form-label">{{ __('fields.at') }}</label>
                            <input type="number" class="form-control" id="characterAt" name="at" required>
                        </div>
                        <div class="mb-3">
                            <label for="characterLevel" class="form-label">{{ __('fields.level') }}</label>
                            <input type="number" class="form-control" id="characterLevel" name="level" required>
                        </div>
                        <div class="mb-3">
                            <label for="characterSpeed" class="form-label">{{ __('fields.speed') }}</label>
                            <input type="number" class="form-control" id="characterSpeed" name="speed" required>
                        </div>
                        <div class="mb-3">
                            <label for="characterInventory" class="form-label">{{ __('fields.inventory') }}</label>
                            <textarea class="form-control" id="characterInventory" name="inventory"></textarea>
                        </div>
                        <div class="mb-3">
                            <label for="characterNotes" class="form-label">{{ __('fields.notes') }}</label>
    ...
    <script>
        ...
        @if($is_owner)
            ...
            function editCharacter(event, id) {
                ...
                event.stopPropagation()
                cur_ind = characters.findIndex(el => el.id == id);
                document.getElementById('characterId').value = id;
                document.getElementById('editCharacterModalLabel').innerHTML = '{{ __('labels.changechar') }} ' + characters[cur_ind].player;
                document.getElementById('characterHp').value = characters[cur_ind].hp;
                document.getElementById('characterAt').value = characters[cur_ind].at;
                document.getElementById('characterLevel').value = characters[cur_ind].level;
                document.getElementById('characterSpeed').value = characters[cur_ind].plus_speed;
                document.getElementById('characterInventory').value = characters[cur_ind].inventory;
                document.getElementById('characterNotes').value = characters[cur_ind].notes || '';
            }
            ...
            function updateCharacter(event) {
                ...
                .then(data => {
                    document.getElementById('at_span_' + characterId).innerHTML = formData.get('at');
                    characters[cur_ind].at = formData.get('at');
                    document.getElementById('hp_span_' + characterId).innerHTML = formData.get('hp');
                    characters[cur_ind].hp = formData.get('hp');
                    document.getElementById('level_span_' + characterId).innerHTML = formData.get('level');
                    characters[cur_ind].level = formData.get('level');
                    characters[cur_ind].plus_speed = formData.get('speed');
                    characters[cur_ind].inventory = formData.get('inventory');
                    characters[cur_ind].notes = formData.get('notes');
